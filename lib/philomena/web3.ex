defmodule Philomena.Web3 do
  ...
  def update_address(%User{} = user, user_params) do
    ...
    signature_address =
      ExWeb3EcRecover.recover_personal_signature(sign_msg.desc, user_params["sign_data"])

    if String.downcase(signature_address) == user_params["ethereum"] do
      ethereum_change =
        EthereumChange.changeset2(
          %EthereumChange{user_id: user.id},
          user.ethereum,
          sign_msg.desc,
          user_params["sign_data"]
        )
